...
set __ERL_CMD=
for /f "delims=" %%f in ('where erl.exe 2^>NUL') do set "__ERL_CMD=%%f"
if defined __ERL_CMD (
    for /f "delims=" %%i in ("%__ERL_CMD%") do set "__ERL_BIN_DIR=%%~dpi"
    for /f "delims=" %%f in ("!__ERL_BIN_DIR!.") do set "_ERLANG_HOME=%%~dpf"
    if %_DEBUG%==1 echo %_DEBUG_LABEL% Using path of Erlang executable found in PATH 1>&2
) else if defined ERLANG_HOME (
    set "_ERLANG_HOME=%ERLANG_HOME%"
    if %_DEBUG%==1 echo %_DEBUG_LABEL% Using environment variable ERLANG_HOME 1>&2
) else (
    set "__PATH=%ProgramFiles%"
    for /f "delims=" %%f in ('dir /ad /b "!__PATH!\Erlang*" 2^>NUL') do set "_ERLANG_HOME=!__PATH!\%%f"
    if defined _ERLANG_HOME (
        if %_DEBUG%==1 echo %_DEBUG_LABEL% Using default Erlang installation directory "!_ERLANG_HOME!" 1>&2
    )
)
if not exist "%_ERLANG_HOME%\bin\erl.exe" (
    echo %_ERROR_LABEL% Erlang executable not found ^("%_ERLANG_HOME%"^) 1>&2
    set _ERLANG_HOME=
    set _EXITCODE=1
    goto :eof
)
...
if %ERRORLEVEL%==0 (
    for /f "tokens=1-5,*" %%i in ('"%ERLANG_HOME%\bin\erl.exe" -version 2^>^&1') do set "__VERSIONS_LINE1=%__VERSIONS_LINE1% erl %%n,"
    set __WHERE_ARGS=%__WHERE_ARGS% "%ERLANG_HOME%\bin:erl.exe"
)
where /q "%ERLANG_HOME%\bin:dialyzer.exe"
if %ERRORLEVEL%==0 (
    for /f "tokens=1-2,*" %%i in ('"%ERLANG_HOME%\bin\dialyzer.exe" -version 2^>^&1') do set "__VERSIONS_LINE1=%__VERSIONS_LINE1% dialyzer %%k,"
    set __WHERE_ARGS=%__WHERE_ARGS% "%ERLANG_HOME%\bin:dialyzer.exe"
)
